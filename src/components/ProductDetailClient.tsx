...
export default function ProductDetailClient({ product, relatedProducts }: ProductDetailClientProps) {
    ...
    const selectedPrice = weightOptions.find(w => w.label === selectedWeight)?.price || product.price;
            ...
            <nav className="bg-[#FAF7F2] py-4">
                <div className="container mx-auto px-4">
                    <ol className="flex items-center text-sm text-[#6B6B6B]">
                        <li><Link href="/" className="hover:text-[#C9A961]">Home</Link></li>
                        <li className="mx-2">/</li>
                        <li><Link href="/shop" className="hover:text-[#C9A961]">Shop</Link></li>
                        <li className="mx-2">/</li>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                ...
                                <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                                    {product.images.map((img, i) => (
                                        <div
                                            ...
                                            className={`w-20 h-20 flex-shrink-0 rounded-md border-2 transition-all cursor-pointer overflow-hidden relative ${mainImage === img ? 'border-[#C9A961]' : 'border-transparent hover:border-gray-300'
                                                }`}
                                        >
                                            <Image
                                                src={img}
                        ...
                        <div>
                                ...
                                <div className="flex gap-2">
                                    {weightOptions.map((option) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-md border-2 text-sm font-medium transition-all ${selectedWeight === option.label
                            ...
                            <div className="flex gap-4 mb-8">
                                ...
                                <button onClick={handleBuyNow} className="btn btn-accent text-lg py-4 px-8">
                                    Buy Now
                                </button>
                            </div>
                    ...
                    <div className="max-w-3xl">
                        {activeTab === 'description' && (
                            <div className="prose prose-lg">
                                ...
                                {product.ingredients && (
                                    <>
                                        <h3 className="text-lg font-semibold mt-6 mb-2" style={{ fontFamily: 'var(--font-heading)' }}>Ingredients</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {Object.entries(product.tastingNotes).map(([key, value]) => (
                                    <div key={key} className="bg-white p-4 rounded-lg">
                                        <h4 className="font-semibold capitalize mb-2 text-[#2C5530]">{key}</h4>
                        ...
                        {activeTab === 'pairings' && product.pairings && (
                            <div>
                                <h3 className="text-lg font-semibold mb-4" style={{ fontFamily: 'var(--font-heading)' }}>Perfect Pairings</h3>
                                <div className="flex flex-wrap gap-3">
                                    {product.pairings.map((pairing, i) => (
                                        <span key={i} className="bg-white px-4 py-2 rounded-full text-sm text-[#6B6B6B] border border-gray-200">
                                            {pairing}
                                        </span>
                                    ))}
                                </div>
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {relatedProducts.map((p) => (
                            <Link key={p.id} href={`/products/${p.id}`} className="product-card bg-white rounded-lg overflow-hidden border border-gray-100 group">
                                ...
                                <div className="p-4">
                                    <h3 className="font-medium mb-1" style={{ fontFamily: 'var(--font-heading)' }}>{p.name}</h3>
                                    <p className="text-[#2C5530] font-semibold">₹{p.price.toLocaleString()}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
