...
export default function CheckoutPage() {
  const { items, getProduct, subtotal, totalWeight, clearCart, isServerDown } = useCart();
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      ...
      if (response.success && response.data && response.data.data) {
        ...
        // 3. Save pending metadata to localStorage
        localStorage.setItem("lepondy_pending_order", JSON.stringify(orderData));
        ...
        addToast(response.message || "Failed to initiate online payment session.", "error");
      }
    } catch (err: any) {
      console.error("Online Payment Error:", err);
      addToast(err.message || "Could not connect to online payment gateway.", "error");
    } finally {
      setIsProcessing(false);
    }
  };
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        {isServerDown && (
          <div className="bg-amber-50 border border-amber-200 text-amber-900 px-5 py-4 rounded-2xl mb-8 flex items-start gap-3 shadow-sm">
            ...
            <div>
              <p className="font-bold text-sm">Checkout Offline</p>
              <p className="text-xs text-amber-700 mt-1">
                The database server is currently offline. Order placements, address creation, and checkout processes are temporarily disabled until the connection is restored.
              </p>
            </div>
          </div>
        )}
                    ...
                    {addresses && addresses.length > 0 && (
                      <div className="mt-8 pt-6 border-t border-gray-50">
                        <button
                          ...
                          className={`w-full py-4 text-lg flex items-center justify-center gap-2 group rounded-xl font-bold transition-all ${
                            isServerDown
                  ...
                  <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <form onSubmit={handleAddNewAddress} className="space-y-6">
                      ...
                      <div className="flex flex-col gap-4">
                        <button
                          type="submit"
                          disabled={isProcessing || isServerDown}
                          className={`w-full py-4 text-lg font-bold rounded-xl transition-all ${
                            isProcessing || isServerDown
                              ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                              : "btn btn-primary shadow-xl shadow-brand-green/10"
                          }`}
                        >
                          {isProcessing
                            ? "Saving..."
            ...
            {step === "payment" && (
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                  <div className="p-5 rounded-2xl border border-gray-200 bg-bg-cream-light/30 text-left mb-6">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-xl">💳</span>
                      <p className="font-bold text-text-primary">Pay Online Securely</p>
                    </div>
                    <p className="text-xs text-gray-500 leading-relaxed">
                      Pay with Credit/Debit Card, UPI, NetBanking, or Wallet using Cashfree Payments.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-4 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-3 ${
                      isProcessing || isServerDown
                        ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                        : "bg-brand-green text-white hover:bg-[#1a3a20] shadow-lg shadow-brand-green/10"
                    }`}
                  >
                    {isProcessing
                      ? "Processing Payment..."
