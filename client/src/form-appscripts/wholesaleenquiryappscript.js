// ============================================================
// Le Pondicherry Cheese — Wholesale Enquiry Form AppScript
...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({ status: 'ok', service: 'Le Cheese Wholesale Enquiry' }))
...
function doPost(e) {
  try {
    ...
    if (!data.businessName || !data.email) {
      throw new Error('Missing required fields: businessName or email');
    }

    const referenceId = 'WHL-' + new Date().getTime();
...
function appendToSheet(data, referenceId, timestamp) {
  ...
  let sheet = ss.getSheetByName('Wholesale Enquiries');
  ...
  const row = [
    ...
    data.businessName,
    data.businessType || '-',
    data.email,     // ← Prevented from #ERROR via setValues with text format
...
function sendEmailNotification(data, referenceId, timestamp) {
  const subject = 'New Wholesale Inquiry | Le Cheese Pondicherry | ' + data.businessName;
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
  ...
  <div style="background-color: #2C5530; padding: 24px 32px;">
    ...
    <p style="color: #ffffff; margin: 4px 0 0 0; font-size: 14px;">New Wholesale Inquiry Received</p>
  </div>

  <div style="padding: 28px 32px; background-color: #FAF7F2;">
    <p style="color: #1A1A1A; font-size: 15px; margin-top: 0;">
      Hello Team,<br><br>
      A new wholesale inquiry has been submitted through the <b>Le Cheese Pondicherry Wholesale Page</b>.
    </p>

    <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
      <tr style="background-color: #2C5530;">
        <td colspan="2" style="padding: 10px 16px; color: #FAB519; font-weight: bold; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em;">BUSINESS DETAILS</td>
      </tr>
      ...
      <tr style="background-color: #ffffff;">
        ...
        <td style="padding: 10px 16px; color: #1A1A1A; font-size: 13px; border-bottom: 1px solid #f0f0f0;">${data.businessName}</td>
      </tr>
      <tr style="background-color: #fafafa;">
        <td style="padding: 10px 16px; font-weight: bold; color: #6B6B6B; font-size: 13px; border-bottom: 1px solid #f0f0f0;">Business Type</td>
        <td style="padding: 10px 16px; color: #1A1A1A; font-size: 13px; border-bottom: 1px solid #f0f0f0;">${data.businessType || '-'}</td>
      </tr>
    </table>

    <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
      <tr style="background-color: #2C5530;">
        <td colspan="2" style="padding: 10px 16px; color: #FAB519; font-weight: bold; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em;">CONTACT DETAILS</td>
      </tr>
      <tr style="background-color: #ffffff;">
        <td style="padding: 10px 16px; font-weight: bold; color: #6B6B6B; width: 38%; font-size: 13px; border-bottom: 1px solid #f0f0f0;">Email</td>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
      <tr style="background-color: #2C5530;">
        <td colspan="2" style="padding: 10px 16px; color: #FAB519; font-weight: bold; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em;">ORDER DETAILS</td>
      </tr>
      <tr style="background-color: #ffffff;">
        <td style="padding: 10px 16px; font-weight: bold; color: #6B6B6B; width: 38%; font-size: 13px;">Expected Monthly Volume</td>
        <td style="padding: 10px 16px; color: #1A1A1A; font-size: 13px; font-weight: bold;">${data.volume || 'Not specified'}</td>
      </tr>
    </table>
    ...
    <div style="margin-top: 24px; padding: 16px; border-left: 4px solid #C9A961; background: #fffbf0;">
      <p style="margin: 0; font-size: 13px; color: #6B6B6B;">
        ...
        <a href="https://docs.google.com/spreadsheets/d/${SHEET_ID}" style="color: #2C5530; font-weight: bold;">📊 View Wholesale Enquiries Sheet</a>
      </p>
    </div>
  ...
  try {
    MailApp.sendEmail({
      to: ADMIN_EMAILS,
      subject: subject,
      body: 'A new wholesale inquiry has been received. Please view this in HTML format.',
      htmlBody: htmlBody
    });
  } catch (err) {
